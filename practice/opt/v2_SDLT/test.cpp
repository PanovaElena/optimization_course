...
bool checkResult(const ParticleEnsemble& particlesEnsemble, FP time) {
	auto particles = particlesEnsemble.getConstAccessor();
	
    FP eps = 1e-4*LIGHT_VELOCITY;
    for (int i = 0; i < particlesEnsemble.getSize(); i++) {
        std::pair<Vector3, Vector3> analyticalSolution = getAnalyticalSolution(time);
		if ((analyticalSolution.first - particles[i].r()).getNorm() > eps ||  // r
            (analyticalSolution.second - particles[i].v()).getNorm() > eps)   // v
            return false;
    }
    return true;
}
...
int main()
{
	...
	auto particle = model.getParticles().getConstAccessor()[0];
